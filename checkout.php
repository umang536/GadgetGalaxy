...
if(isset($_POST['submit'])){
	...
	foreach($_SESSION['cart'] as $key=>$val){
		$resProduct = mysqli_fetch_assoc(mysqli_query($con,"select price from product where id='$key'"));
		$price = $resProduct['price'];
		$qty = isset($val['qty']) ? $val['qty'] : 1;
	...
	if($payment_type=='instamojo'){
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER,
			array("X-Api-Key:71083bac6f587bb3cf5e05fcbdde9733,X-Auth-Token:303a81cd5b5236d00e33a29bfba99acb")
		);
		...
		$payload = Array(
			...
			'redirect_url' => "C:\xampp\htdocs\phpeComPart42\payment_complete.php",
			'send_email' => false,
			'send_sms' => false,
			'email' => $userArr['email'],
